...
//   /* ══════════════════════════════════════════════════════════════
//      PROCTORING HOOK — replaces old showWarning state,
//      warningMessage state, proctoring useEffect, and
//      reportProctoringEvent function entirely.
//   ══════════════════════════════════════════════════════════════ */
//   const { showWarning, warningMessage, dismissWarning, modelsReady } = useProctoring({
//     sessionId,
//     videoRef,
...
//   /* ══════════════════════════════════════════════════════════════
//      GATE SCREEN FACE DETECTION
//      (lightweight pixel check — only used on gate screen
//       to enable the Begin button; full neural proctoring
//       takes over once the interview starts)
//   ══════════════════════════════════════════════════════════════ */
...
//     } catch (err) {
//       console.error("Gate face detection error:", err);
//     }
//   }, 1500);
// };
...
//       setInterviewStarted(true);   // ← this enables the proctoring hook
...
//       await axios.post(
//         `${import.meta.env.VITE_BACKEND_URL}api/subject-interview/${sessionId}/complete`,
//         {},
...
//           <div>
//             <div className="text-lg font-semibold mb-1" style={{ color: "#f9fafb" }}>Verify your setup before starting</div>
//             {subject && <span className="text-xs font-semibold px-2 py-0.5 rounded" style={{ background: "#1e3a5f", color: "#93c5fd" }}>{subject}</span>}
//           </div>
...
//                 disabled={!canBegin || !modelsReady || loading}
//                 className="px-5 py-2 rounded-lg font-semibold text-sm transition-colors"
//                 style={{
//                   background: canBegin && modelsReady && !loading ? "#2563eb" : "#1f2937",
//                   color:      canBegin && modelsReady && !loading ? "#fff"    : "#4b5563",
//                   cursor:     canBegin && modelsReady && !loading ? "pointer" : "not-allowed",
//                 }}
//               >
//                 {loading         ? "Starting…"            :
...
//       {/* Proctoring Warning Modal — dismissWarning from hook */}
//       {showWarning && (
//         <div className="fixed inset-0 flex items-center justify-center z-50" style={{ background: "rgba(0,0,0,0.7)" }}>
...
/**
 * SubjectWiseInterviewRoom.jsx
 *
 * ✅ STT  — MediaRecorder → backend transcribe (YOUR flow from doc9, unchanged)
 * ✅ TTS  — browser SpeechSynthesis (YOUR flow from doc9, unchanged)
 * ✅ Submit answer flow — unchanged from doc9
 * ✅ Audio recording flow — unchanged from doc9
 * ✅ Proctoring — replaced old pixel-variance with useProctoring hook
 *
 * Hook: src/hooks/useProctoring.js
 */
...
const SubjectWiseInterviewRoom = () => {
  ...
  /* ════════════════════════════════════════
     REQUEST PERMISSIONS
     (your original flow — camera + mic,
      face detection now done by hook loop)
  ════════════════════════════════════════ */
  const requestPermissions = async () => {
    setPermStage("requesting");
    try {
  ...
  /* ════════════════════════════════════════
     AUDIO RECORDING — unchanged from doc9
  ════════════════════════════════════════ */
  const startSpeechRecognition = () => {
    try {
      const originalStream = streamRef.current;
  ...
  /* ════════════════════════════════════════
     COMPLETE INTERVIEW — unchanged from doc9
  ════════════════════════════════════════ */
  const completeInterview = async () => {
    try {
      stopTimer();
  ...
  /* ════════════════════════════════════════
     SUBMIT ANSWER — unchanged from doc9
  ════════════════════════════════════════ */
  const submitAnswer = async (autoSubmit = false) => {
    try {
      setLoading(true);
  ...
  /* ════════════════════════════════════════
     EARLY SUBMIT — unchanged from doc9
  ════════════════════════════════════════ */
  const confirmEarlySubmit = async () => {
    setShowSubmitModal(false);
    await completeInterview();
  ...
  if (terminated) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-6 p-8"
      ...
      <p className="text-center max-w-md text-sm leading-6" style={{ color: "#9ca3af" }}>
        Your session was automatically terminated after{" "}
        <strong style={{ color: "#f87171" }}>{MAX_VIOLATIONS} severe proctoring violations</strong>.
      </p>
      {/* <div className="text-xs px-4 py-2 rounded-lg"
        style={{ background: "#1f0a0a", color: "#fca5a5", border: "1px solid #7f1d1d" }}>
        Violations recorded: {violations} — saved to database
      </div> */}
          ...
          <div>
            ...
            {subject && (
              <span className="text-xs font-semibold px-2 py-0.5 rounded"
                style={{ background: "#1e3a5f", color: "#93c5fd" }}>{subject}</span>
            )}
          </div>
  ...
  /* ════════════════════════════════════════
     MAIN INTERVIEW UI — unchanged from doc9
  ════════════════════════════════════════ */
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 p-4 sm:p-6 min-h-[90vh]"
      style={{ background: "#0f1117" }}>
      ...
      <div className="sm:col-span-1 rounded-xl overflow-hidden relative min-h-[200px] sm:min-h-0"
        ...
        {/* Violations badge */}
        {/* {violations > 0 && (
          <div className="absolute top-10 left-3 flex items-center gap-1 rounded px-2 py-1 text-xs font-bold"
      ...
      <div className="sm:col-span-2 rounded-xl p-5 sm:p-6 space-y-5"
        ...
        {/* Recording status box — unchanged from doc9 */}
        {interviewStarted && (
      ...
      {/* Submit Interview Modal — unchanged from doc9 */}
      {showSubmitModal && (
      ...
      {showWarning && (
        <div className="fixed inset-0 flex items-center justify-center z-50"
          ...
          <div className="p-6 rounded-xl w-11/12 max-w-sm text-center"
            ...
            {/* {violations > 0 && (
              <div className="text-xs mb-3 px-3 py-1 rounded inline-block"
                style={{ background: "#1f0a0a", color: "#fca5a5", border: "1px solid #7f1d1d" }}>
