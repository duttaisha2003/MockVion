...
const NebulaBackground = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < 3; i++) {
        ctx.save();
        ctx.globalAlpha = 0.016 + i * 0.005;
        ctx.strokeStyle = `hsl(${210 + i * 22},80%,65%)`;
        ctx.lineWidth = 38 * dpr;
        ctx.beginPath();
        for (let j = 0; j <= 10; j++) {
          const x = (j / 10) * W;
          const y = H * (0.28 + i * 0.1) + Math.sin(j * 0.9 + i * 1.3) * H * 0.055 * Math.sin(i);
          j === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
        }
        ctx.stroke();
        ctx.restore();
      ...
      // Orbs
      const orbDefs = [
        { ox: W * 0.22, oy: H * 0.4,  r: W * 0.3,  hue: 220 },
        { ox: W * 0.75, oy: H * 0.35, r: W * 0.26, hue: 260 },
        { ox: W * 0.5,  oy: H * 0.75, r: W * 0.2,  hue: 210 },
        { ox: W * 0.12, oy: H * 0.18, r: W * 0.16, hue: 240 },
      ];
      orbDefs.forEach(({ ox, oy, r, hue }) => {
        const g = ctx.createRadialGradient(ox, oy, 0, ox, oy, r);
        g.addColorStop(0, `hsla(${hue},90%,65%,0.16)`);
        g.addColorStop(0.5, `hsla(${hue},80%,50%,0.06)`);
      ...
      // Center glow
      const cx = W * 0.5, cy = H * 0.42;
      const glow = ctx.createRadialGradient(cx, cy, 0, cx, cy, W * 0.36);
      glow.addColorStop(0, "rgba(60,130,255,0.055)");
      glow.addColorStop(0.5, "rgba(80,50,180,0.025)");
    ...
    // Draw immediately, then again after a short delay to catch layout shifts
    draw();
